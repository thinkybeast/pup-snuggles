...
export default function Layout({ children, title, active }) {
  return (
    <div className={layoutStyles.pageStyle}>
      <div className={layoutStyles.navBar}>
        <div>
          <img className={layoutStyles.pupLogo} src={logo} alt="Pup Snuggles" />
        </div>
        <h1 className={layoutStyles.navTitle}>Pup Snuggles</h1>
        <a
        ...
        <div className={layoutStyles.gatsbyIcon}>{GatsbyIcon}</div>
        <p className={layoutStyles.poweredBy}>
          Powered by{" "}
          <a
      ...
      <h1 style={{ marginLeft: `30px` }} className={layoutStyles.subTitle}>
        {title}
      </h1>
      <div>{children}</div>
    </div>
